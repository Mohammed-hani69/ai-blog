...
export const DashboardLayout: React.FC<DashboardLayoutProps> = ({ activeTab, setActiveTab, children, onLogout, adminProfile, supabaseAvailable }) => {
      ...
      <main className="flex-1 mr-64 transition-all duration-300">
        ...
        <header className="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-8 sticky top-0 z-40">
          ...
          <div className="flex items-center space-x-4 space-x-reverse">
            {supabaseAvailable ? (
              <div className="bg-emerald-50 text-emerald-700 px-3 py-1 rounded-lg text-sm border border-emerald-100">قاعدة بيانات مشتركة (Backend)</div>
            ) : (
              <div className="bg-yellow-50 text-yellow-700 px-3 py-1 rounded-lg text-sm border border-yellow-100">التخزين محلي فقط (Local)</div>
            )}
